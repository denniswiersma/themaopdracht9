...
\begin{table}[thb]
\caption{\label{DefaultComparison}Comparison of ML algorithms using default settings based on elapsed testing time, accuracy, False Negative Rate, F-measure, and ROC Area, using ZeroR as a baseline.}
\footnotesize
...
`nearestNeighbors` = 5 \
`percentage` = 450 \
This means we select the right instances to increase, base new instances on it's 5 nearest neighbors, and increase the number of instances by 450% therefore balancing the `yes` and `no` labels. We can then save this dataset and return to the Weka experimenter. After completing all these steps we get the following results:

\begin{table}[thb]
\caption{\label{DefaultSMOTEComparison}Comparison of ML algorithms using default settings based on elapsed testing time, accuracy, False Negative Rate, F-measure, and ROC Area after performing SMOTE on the dataset, using ZeroR as a baseline.}
\footnotesize
{\centering \begin{tabular}{lr||r@{\hspace{0.1cm}}cr@{\hspace{0.1cm}}cr@{\hspace{0.1cm}}cr@{\hspace{0.1cm}}cr@{\hspace{0.1cm}}cr@{\hspace{0.1cm}}cr@{\hspace{0.1cm}}c}\\
\hline
\textbf{Algorithm} & \textit{ZeroR} & \textit{OneR} & & \textit{J48} & & \textit{RandomForest} & & \textit{IBk} & & \textit{NaiveBayes} & & \textit{SimpleLogistic} & & \textit{SMO} & \\
\hline
\hline
\textbf{Elapsed time testing} & 0.00 & 0.00 &         & 0.00 &         & 0.02 & $\circ$ & 0.12 & $\circ$ & 0.00 & $\circ$ & 0.00 & $\circ$ & 0.00 & $\circ$\\
\hline
\textbf{Accuracy} & 50.36 & 87.90 & $\circ$ & 85.61 & $\circ$ & 86.63 & $\circ$ & 77.16 & $\circ$ & 62.38 & $\circ$ & 67.33 & $\circ$ & 67.22 & $\circ$\\
\hline
\textbf{False Negative Rate} & 0.00 & 0.00 &         & 0.10 & $\circ$ & 0.13 & $\circ$ & 0.30 & $\circ$ & 0.22 & $\circ$ & 0.33 & $\circ$ & 0.36 & $\circ$\\
\hline
\textbf{F-measure} & 0.67 & 0.89 & $\circ$ & 0.86 & $\circ$ & 0.87 & $\circ$ & 0.75 & $\circ$ & 0.67 &         & 0.67 &         & 0.66 &        \\
\hline
\textbf{ROC Area} & 0.50 & 0.88 & $\circ$ & 0.88 & $\circ$ & 0.94 & $\circ$ & 0.77 & $\circ$ & 0.70 & $\circ$ & 0.73 & $\circ$ & 0.67 & $\circ$\\
\hline
\multicolumn{16}{c}{$\circ$, $\bullet$ statistically significant increase or decrease}\\
\end{tabular} \footnotesize \par}
\end{table}
...
\hline
\textbf{Accuracy} & 50.36 & 87.90 & $\circ$ & 85.73 & $\circ$ & 82.11 & $\circ$ & 77.16 & $\circ$ & 61.80 & $\circ$ & 61.94 & $\circ$ & 67.22 & $\circ$\\
\hline
\textbf{False Negative Rate} & 0.00 & 0.00 &         & 0.10 & $\circ$ & 0.03 & $\circ$ & 0.30 & $\circ$ & 0.15 & $\circ$ & 0.12 & $\circ$ & 0.36 & $\circ$\\
\hline
\textbf{F-measure} & 0.67 & 0.89 & $\circ$ & 0.86 & $\circ$ & 0.85 & $\circ$ & 0.75 & $\circ$ & 0.69 & $\circ$ & 0.70 & $\circ$ & 0.66 &        \\
\hline
\textbf{ROC Area} & 0.50 & 0.88 & $\circ$ & 0.86 & $\circ$ & 0.82 & $\circ$ & 0.77 & $\circ$ & 0.62 & $\circ$ & 0.62 & $\circ$ & 0.67 & $\circ$\\
\hline
\multicolumn{16}{c}{$\circ$, $\bullet$ statistically significant increase or decrease}\\
\end{tabular} \footnotesize \par}
\end{table}
